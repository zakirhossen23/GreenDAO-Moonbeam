		...
		<>
			...
			<div className={`${styles.container} flex flex-col items-center gap-8`}>
				<div className={`${styles.title} flex flex-col gap-2`}>
					...
					<Loader element={<h1 className="text-moon-32 font-bold pb-2" style={{ width: "78%" }}>
							{IdeasURI.Title}
						</h1>} width={'80%'} /> 		
						<a
					...
					<div>
					<Loader element={<a className="text-piccolo" name="dateleft" date={IdeasURI.End_Date}>
							{LeftDate(IdeasURI.End_Date)}
						</a>} width={'80%'} /> 			
						
					</div>
					<Loader element={<p>{IdeasURI.Description} </p>} width={'80%'} /> 
					
					<Loader element={	<div className="flex">Voted: {IdeasURI.voted}  </div>} width={'100%'}/>
					<Loader element={<p>Wallet: {IdeasURI.wallet}</p>} width={'100%'}/>
				</div>
				...
				<>
					<div className={`flex gap-8`}>
						<Loader type="rounded" element={(imageList.length > 1 ? (
							<>
